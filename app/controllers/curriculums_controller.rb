class CurriculumsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to curriculums_path, status: :see_other, notice: "Curriculum was successfully destroyed." }
  ...
  private
    ...
    def set_curriculum
      @curriculum = Curriculum.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def curriculum_params
      params.require(:curriculum).permit(:title, :program_model, lessons_attributes: [ :id, :title, :content, :duration, :_destroy, activities_attributes: [ :id, :name, :_destroy ] ])
    end
end
